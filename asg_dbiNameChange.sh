...
2. Description: run "asg_dbiNameChange".
3. Author: Anthony Davis
4. Date: 10/07/2025
5. Usage: asg_dbiNameChange
'
...
for i in asgcon1; do
    scp /usr/tmp/dbiNameChanges $i:/usr/tmp/ >/dev/null 2>&1
    ssh $i 'chmod 770 /usr/tmp/dbiNameChanges'
done

#for i in $(cat $SITE | grep -v ^# | grep -v ehr2 | grep -v qcci01); do
#    scp /ccrun/cci/run/dbiNameChanges $i:/usr/tmp/ >/dev/null 2>&1
#    ssh $i 'chmod 770 /usr/tmp/dbiNameChanges'
#done
...
for i in asgcon1; do
    ...
    ssh -T $i << 'ENDSSH'
        ...
        echo "Completed at $(date)" | tee -a "$logname"
